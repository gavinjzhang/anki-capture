...
export interface UserSettings {
  ...
  daily_llm_usage: number | null;
  daily_llm_limit: number | null;
}
...
export async function updateLLMKey(
  ...
  model: string,
  apiKey: string,
): Promise<UserSettings & { validated: boolean }> {
  return request('/api/settings', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function deleteLLMKey(): Promise<{ success: boolean }> {
